    ...
    public function action_hre()
    {
        ...
        if ($auth->logged_in() && isset($_GET['code'])) {
            ...
            foreach ($hojaruta as $rs) 
            {
                ...
                $pdf->Cell(33, 7, 'REFERENCIA:', 1, FALSE, 'L', true);
                //$pdf->Ln(0);
                ...
                $pdf->SetFontSize(8);//$proveido = utf8_decode("");//proveidoif ($pro == 0) {$proveido = "";}
                /*$pdf->Cell(120, 20, utf8_decode($rs->referencia),0,false, 'L');//$pa = $pdf->GetY();
                $pdf->SetXY(10, 65);
...
//    $pdf->Cell(65, 5, $rs->sigla, 'TR',FALSE,'C');
                ...
                foreach ($proveidos as $p) {
                    ...
                    $pdf->MultiCell(144, 5, utf8_decode($proveido), 'RL', 'L');
                    $pdf->SetXY(10, $pa);
